...
// Crear usuario (seller, waiter, kitchen)
if (isset($_POST['nuevo_usuario'])) {
  $nombre = trim($_POST['nombre']);
  $tienda_id = intval($_POST['tienda_id']);
  $rol = $_POST['rol'];

  if (!empty($nombre) && $tienda_id > 0 && in_array($rol, ['seller', 'waiter', 'kitchen'])) {
    $password_hash = null;
    $status = "active";

    $stmt = $conexion->prepare("INSERT INTO users (username, password_hash, role, store_id, status) VALUES (?,?,?,?,?)");
    $stmt->bind_param("sssis", $nombre, $password_hash, $rol, $tienda_id, $status);
    $stmt->execute();
    $stmt->close();
...
if (isset($_POST['cambiar_modo']) && $role === 'user') {
  ...
  if ($nuevo_modo === 'simple' || $nuevo_modo === 'controlado' || $nuevo_modo === 'pedidos') {
          ...
          <form method="post" class="row g-3 mb-3">
            ...
            <div class="col-md-3">
              <select name="tienda_id" class="form-control" required>
                ...
                <?php
                $tiendas = $conexion->query("SELECT id, name FROM stores");
                    ...
                    <button name="modo" value="controlado" onclick="return confirm('¿Seguro que quieres cambiar el modo de la tienda? En modo controlado deberas actualizar tus insumos antes de poder realizar pedidos');"
                      ...
                      <div class="card-body p-3">
                        ...
                        <small class="text-muted">
                          Descuenta automáticamente los insumos al realizar pedidos y calcula cuántos productos puedes
                          preparar según tu inventario.
                        </small>
                  ...
                  <div class="col-md-6">
                    <button name="modo" value="simple" onclick="return confirm('¿Seguro que quieres cambiar el modo de la tienda?');"
                      ...
                      <div class="card-body p-3">
                        ...
                        <small class="text-muted">
                          El inventario se usa solo para registrar costos de insumos. Los pedidos no afectan el stock ni
                          las recetas.
                        </small>
                  ...
                  <div class="col-md-6">
                    <button name="modo" value="pedidos" onclick="return confirm('¿Seguro que quieres cambiar el modo de la tienda? En modo pedidos, no se tendran en cuenta los insumos al realizar pedidos');"
                      ...
                      <div class="card-body p-3">
                        ...
                        <small class="text-muted">
                          Solo permite crear productos, registrar pedidos y gestionar estados (cocina/meseros).
                        </small>

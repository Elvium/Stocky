...
require 'verificar_sesion.php'; // tu conexión a la BD
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['delete_id'])) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $new_quantity = ($modo === 'simple') ? 0 : $row['quantity'] + $total_quantity;
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $insert->bind_param("iissidsi", $store_id, $user_id, $name, $brand, $total_quantity, $price, $unit, $limite);

        $insert->execute();

        $new_id = $conexion->insert_id; // ID del nuevo insumo

        // 🔹 Preparar variables antes de bind_param (porque bind_param solo acepta variables)
        $quantity_log = ($modo === 'simple') ? NULL : $total_quantity;
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="name" id="nameSelect" onchange="toggleNameInput(this)">
                            <option value="" disabled selected hidden>Selecciona/crea un insumo</option>

                            <option value="other">Nuevo insumo</option>
                            <?php while ($n = $names_result->fetch_assoc()): ?>
                                <option value="<?= htmlspecialchars($n['name']) ?>"><?= htmlspecialchars($n['name']) ?>
                                </option>
                            <?php endwhile; ?>

                        </select>
                        <input type="text" name="name" id="nameInput" class="form-control mt-2 d-none"
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="brand" id="brandSelect" onchange="toggleBrandInput(this)">
                            <option value="" disabled selected hidden>Selecciona/crea una marca o proveedor</option>
                            <option value="other">Nueva Marca / Proveedor</option>
                            <?php while ($b = $brands_result->fetch_assoc()): ?>
                                <option value="<?= htmlspecialchars($b['brand']) ?>"><?= htmlspecialchars($b['brand']) ?>
                                </option>
                            <?php endwhile; ?>

                        </select>
                        <input type="text" name="brand" id="brandInput" class="form-control mt-2 d-none"

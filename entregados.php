...
if (isset($_GET['pagar']) && isset($_GET['metodo'])) {
  $metodo = $_GET['metodo'];
  ...
  <main class="container my-5">
    ...
    <form method="get" class="mb-3 d-flex align-items-center gap-2">
      ...
      <select name="estado" id="estado" class="form-select form-select-sm" onchange="this.form.submit()">
        ...
        <option value="Canceled" <?= $estadoFiltro === 'Canceled' ? 'selected' : '' ?>>Cancelado</option>
      </select>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex justify-content-center gap-2">
                  ...
                  <?php if (!$paid && !$isCanceled): ?>
                    <button class="btn btn-sm btn-primary btn-pagar" data-id="<?= $row['id'] ?>">
                      Pagado
                    </button>
                  <?php else: ?>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button id="confirmarPago" class="btn btn-success">Confirmar</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    document.querySelectorAll('.btn-pagar').forEach(btn => {
      btn.addEventListener('click', () => {
        saleIdSeleccionado = btn.dataset.id;
